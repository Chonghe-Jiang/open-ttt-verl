...
The same 3xH200 recipe can be launched on the local Slurm cluster through
Apptainer:

```bash
scripts/launch_polyomino_h200_gpt_oss_120b_group16_h200_tuned.sh
```

The launcher first runs a CPU smoke gate: container package imports, local
FrontierCS judge startup, one seeded Polyomino verifier call, and
`--prepare-only`. If that passes, it submits
`scripts/slurm_polyomino_h200_gpt_oss_120b_group16_h200_tuned.sh`, which uses
GPU `2` for the local `openai/gpt-oss-120b` vLLM execution server and GPUs
`0,1` for the Qwen3-8B guidance actor. The modal YAML is reused unchanged; the
runner maps local directories into the Modal paths:

```text
.modal_local_runs  -> /runs
.modal_local_cache -> /cache
/home/qua/code/reference/Frontier-CS -> /opt/Frontier-CS
```

If `flash_attn` is missing from the Apptainer image, the local runner downloads
the same H200-compatible wheel used by Modal and installs it into the user
Python site-packages. If the prebuilt wheel install fails, it falls back to
building `flash-attn==2.8.3` locally.
