from __future__ import annotations

import builtins
import multiprocessing as mp
import queue
import traceback
from dataclasses import dataclass
from typing import Any

import numpy as np


@dataclass
class SandboxResult:
    output: Any | None
    error: str | None


def _worker(code: str, kwargs: dict[str, Any], result_queue: mp.Queue) -> None:
    try:
        globals_dict = {
            "__builtins__": builtins.__dict__,
            "np": np,
            "numpy": np,
        }
        exec(code, globals_dict)
        run_fn = globals_dict.get("run")
        if run_fn is None:
            raise ValueError("Program must define run(seed=42, budget_s=..., **kwargs)")
        result_queue.put(SandboxResult(output=run_fn(**kwargs), error=None))
    except Exception:
        result_queue.put(SandboxResult(output=None, error=traceback.format_exc()))


def evaluate_python_code(
    code: str,
    *,
    timeout_s: int,
    seed: int = 42,
    budget_s: int = 1,
    n_points: int = 256,
) -> SandboxResult:
    ctx = mp.get_context("spawn")
    result_queue: mp.Queue = ctx.Queue(maxsize=1)
    kwargs = {"seed": seed, "budget_s": budget_s, "n_points": n_points}
    process = ctx.Process(target=_worker, args=(code, kwargs, result_queue))
    process.start()
    process.join(timeout_s)
    if process.is_alive():
        process.terminate()
        process.join(1)
        if process.is_alive():
            process.kill()
        return SandboxResult(output=None, error=f"Timed out after {timeout_s}s")
    try:
        return result_queue.get_nowait()
    except queue.Empty:
        return SandboxResult(output=None, error=f"Process exited with code {process.exitcode}")

