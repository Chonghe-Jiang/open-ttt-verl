...
class GuidanceLibrary:
    ...
    def __init__(
        ...
        *,
        ...
        self._puct_T: int = 0
        self._loaded_runtime_config_keys: set[str] = set()
    ...
    def assert_runtime_config(
        ...
        *,
        ...
        missing_runtime_keys = set(actual) - self._loaded_runtime_config_keys
        if self.discover_compat and missing_runtime_keys:
            raise ValueError(
                f"Guidance library runtime config mismatch for {self.path}: archive predates "
                f"Discover-compatible accounting and is missing {sorted(missing_runtime_keys)}. "
        ...
        if mismatches:
            raise ValueError(
                ...
                + "; ".join(mismatches)
                + ". Existing run libraries are immutable with respect to sampling config; use a new output directory."
            )
        if self.discover_compat:
    ...
    def _assert_group_accounting(self) -> None:
        ...
        if self._puct_T != expected_puct_updates:
            ...
            errors.append(
                f"puct_T={self._puct_T}, expected one update for each of {expected_puct_updates} {unit}s"
            )
        if errors:
            ...
            with self._file_lock():
                ...
                if group is not None:
                    ...
                    if require_solution and not self._node_has_solution(selected):
                        raise RuntimeError(
                            f"Existing group {group_uid!r} is attached to node {selected.id!r}, "
                            "which has no solution code."
                        )
                    return selected
    ...
    def ensure_pristine_root_count(self, count: int) -> None:
        """Replicate seed roots so same-batch groups start from independent lineages."""
        ...
        with self._thread_lock:
            with self._file_lock():
                ...
                while len(roots) < target_count:
                    ...
                    if source.entry_id and source.entry_id in self._entries:
                        cloned_entry = LibraryEntry.from_dict(self._entries[source.entry_id].to_dict())
        ...
        with self._thread_lock:
            with self._file_lock():
                ...
                should_archive = (
                    ...
                    or (entry.verifier_status == "valid" and entry.verifier_raw_score is not None)
                )
                ...
                if self.discover_compat:
                    self._record_puct_rollout(parent.id, child_value=puct_child_value)
    ...
    def _refresh_best(self) -> None:
        candidates = [node for node in self._nodes.values() if self._node_is_archive_candidate(node)]
        if not candidates:
            self._best_node_id = None
            return
        self._best_node_id = max(candidates, key=lambda node: (node.value, node.id)).id
    ...
    def _save(self) -> None:
        ...
        try:
            ...
            self._loaded_runtime_config_keys = set(self._runtime_config())
        finally:
            if temp_path.exists():
                temp_path.unlink()
    ...
    def _load(self) -> None:
        ...
        self._loaded_runtime_config_keys = set(config)
        ...
        self.puct_q_mode = normalize_puct_q_mode(config.get("puct_q_mode", self.puct_q_mode))
        ...
        self.discover_compat = bool(config.get("discover_compat", self.discover_compat))
        self.groups_per_batch = int(config.get("groups_per_batch", self.groups_per_batch))
        self.score_direction = self._normalize_score_direction(config.get("score_direction", self.score_direction))
