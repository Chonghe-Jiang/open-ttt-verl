...
class TriMulEnvironmentError(RuntimeError):
    """Raised when the official Modal H100 evaluator is unavailable."""
...
def evaluate_trimul_solution(
    ...
    *,
    ...
    provider = str(judge_config.get("provider", "modal_http")).strip().lower()
    if provider not in {"http", "modal_http"}:
        raise TriMulEnvironmentError(
            "TriMul evaluation is intentionally remote-only; provider must be 'modal_http'"
        )
...
def _evaluate_with_http(code: str, *, timeout_s: int, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not endpoint:
        raise TriMulEnvironmentError(
            f"TriMul Modal verifier requires endpoint or environment variable {endpoint_env}"
        )
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            payload.setdefault("provider", "modal_http")
