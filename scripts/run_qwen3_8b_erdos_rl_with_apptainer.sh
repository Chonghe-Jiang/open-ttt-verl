...
SAVE_CKPT=${SAVE_CKPT:-${WORKSPACE}/ckpt/Qwen3-8B_erdos_lora}
...
KILL_STALE_PROCESSES=${KILL_STALE_PROCESSES:-0}
...
INNER_SCRIPT=${INNER_SCRIPT:-${WORKSPACE}/tmp/run_qwen3_8b_erdos_rl_inner_${RUN_ID}.sh}
...
if [ "${SGLANG_CONTEXT_LENGTH}" -le "${ROLLOUT_MAX_RESPONSE_LEN}" ]; then
  echo "Invalid context config: SGLANG_CONTEXT_LENGTH must be greater than ROLLOUT_MAX_RESPONSE_LEN." >&2
  echo "Got ${SGLANG_CONTEXT_LENGTH} <= ${ROLLOUT_MAX_RESPONSE_LEN}" >&2
  exit 1
fi
...
test -f "${REF_CKPT}/latest_checkpointed_iteration.txt" || {
  echo "Missing converted Megatron checkpoint at ${REF_CKPT}." >&2
  echo "Wait for scripts/download_convert_qwen3_8b_2gpu.slurm to finish successfully before RL." >&2
  exit 1
}
test -f "${REPO_DIR}/erdos_slime/ttt_discover/__init__.py" || {
  echo "Missing ${REPO_DIR}/erdos_slime/ttt_discover." >&2
  echo "This TTT RL run needs the local Erdos task/archive/sandbox utilities." >&2
  exit 1
}
test -e "${CONTAINER_PATH}" || {
  echo "Missing container: ${CONTAINER_PATH}" >&2
  echo "Run scripts/build_slime_sandbox.sh first, or set CONTAINER_PATH." >&2
  exit 1
}
...
echo "=== qwen3-8b erdos rl container ==="
echo "time: \$(date -Is)"
echo "host: \$(hostname)"
echo "pwd: \$(pwd)"
...
if not torch.cuda.is_available() or count < required:
    raise SystemExit(f"Need at least {required} visible CUDA devices, got {count}. Re-submit the Slurm job with enough GPUs.")
PY
