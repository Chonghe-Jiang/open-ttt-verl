...
group_size=16
config="guidance_ttt/config/polyomino_b200_5gpu_qwen36_27b_gpt_oss_120b_batch8_group8_code_delta_prompt8192_entropic_blended_500step.yaml"
...
smoke_output="outputs/guidance_ttt/qwen36_27b_guidance_g16_code_delta_prompt8192_blended_smoke_${tag}"
formal_output="outputs/guidance_ttt/qwen36_27b_guidance_g16_code_delta_prompt8192_blended_two_day_${tag}"
...
smoke_args=(--parsable --job-name="q36-27b-g16-codeblend-smoke" --time=04:00:00)
...
day1=$(SHARED_OUTPUT_DIR="${formal_output}" STAGE=day1 \
  sbatch --parsable --job-name="q36-27b-g16-codeblend-d1" --dependency="afterok:${smoke}" \
  --export="${common_export}" \
  scripts/slurm_polyomino_b200_5gpu_qwen36_27b_guidance_code_delta_prompt8192_group8_stage.sbatch)
day2=$(SHARED_OUTPUT_DIR="${formal_output}" STAGE=day2 \
  sbatch --parsable --job-name="q36-27b-g16-codeblend-d2" --dependency="afterok:${day1}" \
  --export="${common_export}" \
  scripts/slurm_polyomino_b200_5gpu_qwen36_27b_guidance_code_delta_prompt8192_group8_stage.sbatch)

echo "code_delta blended: group_size=${group_size} smoke=${smoke} day1=${day1} day2=${day2} output=${formal_output}"
