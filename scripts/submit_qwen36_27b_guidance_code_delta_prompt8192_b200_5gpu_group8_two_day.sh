...
smoke_output="outputs/guidance_ttt/qwen36_27b_guidance_g${group_size}_code_delta_prompt8192_smoke_${tag}"
formal_output="outputs/guidance_ttt/qwen36_27b_guidance_g${group_size}_code_delta_prompt8192_two_day_${tag}"
smoke_args=(--parsable --job-name="q36-27b-g${group_size}-code8192-smoke" --time=04:00:00)
...
day1=$(sbatch --parsable --job-name="q36-27b-g${group_size}-code8192-d1" --dependency="afterok:${smoke}" \
  --export=ALL,SHARED_OUTPUT_DIR="${formal_output}",STAGE=day1,GROUP_SIZE_OVERRIDE="${group_size}" \
  scripts/slurm_polyomino_b200_5gpu_qwen36_27b_guidance_code_delta_prompt8192_group8_stage.sbatch)
day2=$(sbatch --parsable --job-name="q36-27b-g${group_size}-code8192-d2" --dependency="afterok:${day1}" \
  --export=ALL,SHARED_OUTPUT_DIR="${formal_output}",STAGE=day2,GROUP_SIZE_OVERRIDE="${group_size}" \
  scripts/slurm_polyomino_b200_5gpu_qwen36_27b_guidance_code_delta_prompt8192_group8_stage.sbatch)

echo "code_delta_prompt8192: group_size=${group_size} smoke=${smoke} day1=${day1} day2=${day2} output=${formal_output}"
