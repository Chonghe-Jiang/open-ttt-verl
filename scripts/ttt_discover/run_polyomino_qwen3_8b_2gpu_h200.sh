...
# Two-GPU H200 TTT-Discover/Frontier-CS Polyomino launch for Qwen/Qwen3-8B.
# Override GPUS, CONFIG, HF_HOME, MODEL_PATH, or ATTN_IMPL from the shell.

GPUS="${GPUS:-0,1}"
CONFIG="${CONFIG:-verl_ttt_discover/config/polyomino_2gpu_h200_qwen3_8b_g4_n16.yaml}"
...
python -m verl_ttt_discover.main_polyomino \
  --config "${CONFIG}" \
  "${overrides[@]}" \
  "$@"
