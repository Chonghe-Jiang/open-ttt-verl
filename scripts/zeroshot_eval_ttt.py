"""
Zero-shot eval of TTT-Discover trained LoRA on the OTHER 18 tasks (excluding train task).
Same protocol as base_eval_iterative.py — iterative tool calling, 8 rollouts/task.
Uses the same vLLM server but with --lora-name (vLLM serves both base model
and LoRA adapter via /v1/chat/completions with model=<lora-name>).
"""
import argparse
import json
import sys
...
def run_one_rollout(*, task, rollout_idx, server, model, tasks_json, frontier_root,
    ...
    print(f"[zs] {task} r{rollout_idx} score={result['final_score']:.4f} "
...
def main():
    ...
    ap.add_argument("--model", required=True, help="LoRA name registered in vLLM")
    ap.add_argument("--exclude-task", required=True, help="train task to exclude")
    ...
    all_tasks = [t for t in all_task_names(tasks_json) if t != args.exclude_task]
    ...
    print(f"[zs] shard {args.shard_id}/{args.num_shards}: {len(jobs)} rollouts "
          f"(of {len(all_jobs)} total) on {len(all_tasks)} tasks (excluding {args.exclude_task})",
          flush=True)
    ...
    print(f"[zs] shard done in {time.time()-t_start:.0f}s", flush=True)
