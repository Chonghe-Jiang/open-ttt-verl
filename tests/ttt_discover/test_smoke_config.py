...
def test_two_gpu_h200_qwen3_8b_g4_n16_config_matches_requested_modal_shape():
    config = _load_config("erdos_2gpu_h200_qwen3_8b_g4_n16.yaml")
    overrides = config["verl_overrides"]

    assert config["run"]["model_path"] == "Qwen/Qwen3-8B"
    assert config["run"]["output_dir"] == "outputs/ttt_erdos/2gpu_h200_qwen3_8b_g4_n16"
    assert config["run"]["experiment_name"] == "erdos_2gpu_h200_qwen3_8b_g4_n16"
    assert config["run"]["n_gpus_per_node"] == 2
    assert config["run"]["tensor_model_parallel_size"] == 2
    assert config["run"]["num_initial_states"] == 4
    assert config["run"]["num_steps"] == 50
    ...
    assert "actor_rollout_ref.rollout.max_model_len=32768" in overrides
    assert "actor_rollout_ref.rollout.max_num_seqs=128" in overrides
    assert "actor_rollout_ref.rollout.checkpoint_engine.update_weights_bucket_megabytes=1280" in overrides
    assert "actor_rollout_ref.rollout.agent.num_workers=16" in overrides
